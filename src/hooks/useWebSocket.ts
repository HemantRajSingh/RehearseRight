...
export const useWebSocket = ({ onMessage, onError, onClose }: WebSocketHookOptions) => {
          ...
          .then(stream => {
            ...
            processor.onaudioprocess = (e) => {
              if (ws.current?.readyState === WebSocket.OPEN) {
                ...
                const audioData = btoa(String.fromCharCode.apply(null, new Uint8Array(inputData.buffer)));
                ws.current.send(JSON.stringify({
                  type: 'audio',
                  data: audioData
                }));
